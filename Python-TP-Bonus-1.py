...
class Bebedor(threading.Thread):
    ...
    def tomarLata(self):
        ...
        listaHeladeras[self.heladeraElegida].cantLatas -= 1
        self.cantMaximaLatas -= 1

    def tomarBotella(self):
        logging.info(
            f'Bebedor {self.numero}: Me tomo una botella de cerveza')
        listaHeladeras[self.heladeraElegida].cantBotellas -= 1
    ...
    def run(self):
        ...
        while self.cantMaximaBotellas > 0 or self.cantMaximaLatas > 0:
            if listaHeladeras[self.heladeraElegida].cantLatas > 0 and self.cantMaximaLatas > 0:
                self.tomarLata()
            if listaHeladeras[self.heladeraElegida].cantBotellas > 0 and self.cantMaximaBotellas > 0:
                self.tomarBotella()
        logging.info(
